...
class TD3:
    ...
    @torch.no_grad()
    def calc_q_target(self, next_state, reward, done):
        ta = self.target_actor(next_state)
        q_target = reward + GAMMA * (1 - done) * torch.min(
            self.target_critic_1(next_state, ta),
            self.target_critic_2(next_state, ta)
        )
        return q_target
    ...
    def update(self, transition):
        ...
        if len(self.replay_buffer) > BATCH_SIZE * 16:
            ...
            q_target = self.calc_q_target(next_state, reward, done)

            # Update critic
            self.critic_1_optim.zero_grad()
            self.critic_2_optim.zero_grad()
            loss_1 = F.mse_loss(self.critic_1(state, action), q_target)
            loss_2 = F.mse_loss(self.critic_2(state, action), q_target)
            loss_1.backward()
            loss_2.backward()
...
if __name__ == "__main__":
    ...
    for i in range(TRANSITIONS):
        ...
        if i % 10000 == 0:
            ...
            reward = np.mean(rewards) - 3 * np.std(rewards)
